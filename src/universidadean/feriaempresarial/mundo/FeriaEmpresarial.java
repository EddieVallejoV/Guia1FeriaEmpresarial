...
public class FeriaEmpresarial {
    ...
    private int numExpositores(String nombreEmpresa) {
        for (Empresa e : empresas) {
            if (e.darNombre().equals(nombreEmpresa) &&
                    e.darTipo().equals(Empresa.EXPOSITORA)) {
    ...
    public String metodo1() {
        /* ZONA OCCIDENTAL */
        List<Integer> zonaocc = new ArrayList<Integer>();
        for (Puesto p : puestos) {
            if (p.estaOcupado()) {
                    if (p.darZona().equals(Puesto.ZONA_OCCIDENTE)) {
                        Integer a = Integer.valueOf(numExpositores(p.darNombreEmpresa()));
                        zonaocc.add(a);
                    }
            }
        }

        /*System.out.println(zonaocc);*/
        Integer[] azonaocc = zonaocc.stream().toArray(Integer[]::new);
        /*System.out.println(Arrays.toString(azonaocc));*/
        int totalocc=0;
        for(int contador = 0; contador< azonaocc.length;contador++){
            totalocc += azonaocc[contador];
        }
        System.out.println("Ocupadas Occidental: " + totalocc );

        /*Zona Oriente*/

        List<Integer> zonaori = new ArrayList<Integer>();
        for (Puesto p : puestos) {
            if (p.estaOcupado()) {
                if (p.darZona().equals(Puesto.ZONA_ORIENTE)) {
                    Integer b = Integer.valueOf(numExpositores(p.darNombreEmpresa()));
                    zonaori.add(b);

                }
            }
        }
        /*System.out.println(zonaori);*/
        Integer[] azonaori = zonaori.stream().toArray(Integer[]::new);
        /*System.out.println(Arrays.toString(azonaori));*/
        int totalori=0;
        for(int contador = 0; contador< azonaori.length;contador++){
            totalori += azonaori[contador];
        }
        System.out.println("Ocupadas Oriental: " + totalori );

        /* ZONA NORTE */
        List<Integer> zonanor = new ArrayList<Integer>();
        for (Puesto p : puestos) {
            if (p.estaOcupado()) {
                if (p.darZona().equals(Puesto.ZONA_NORTE)) {
                    Integer c = Integer.valueOf(numExpositores(p.darNombreEmpresa()));
                    zonanor.add(c);
                }
            }
        }

        /*System.out.println(zonanor);*/
        Integer[] azonanor = zonanor.stream().toArray(Integer[]::new);
        /*System.out.println(Arrays.toString(azonanor));*/
        int totalnor=0;
        for(int contador = 0; contador< azonanor.length;contador++){
            totalnor += azonanor[contador];
        }
        System.out.println("Ocupadas Norte: " + totalnor );

        /* ZONA SUR */
        List<Integer> zonasur = new ArrayList<Integer>();
        for (Puesto p : puestos) {
            if (p.estaOcupado()) {
                if (p.darZona().equals(Puesto.ZONA_SUR)) {
                    Integer d = Integer.valueOf(numExpositores(p.darNombreEmpresa()));
                    zonasur.add(d);
                }
            }
        }
        ...
        Integer[] azonasur = zonasur.stream().toArray(Integer[]::new);
        /*System.out.println(Arrays.toString(azonasur));*/
        int totalsur=0;
        for(int contador = 0; contador< azonasur.length;contador++){
            totalsur += azonasur[contador];
        }
        System.out.println("Ocupadas Sur: " + totalsur );

        /* ZONA CENTRAL */
        List<Integer> zonacen = new ArrayList<Integer>();
        for (Puesto p : puestos) {
            if (p.estaOcupado()) {
                if (p.darZona().equals(Puesto.ZONA_CENTRO)) {
                    Integer e = Integer.valueOf(numExpositores(p.darNombreEmpresa()));
                    zonacen.add(e);
                }
            }
        }

        /*System.out.println(zonacen);*/
        Integer[] azonacen = zonacen.stream().toArray(Integer[]::new);
        /*System.out.println(Arrays.toString(azonacen));*/
        int totalcen=0;
        for(int contador = 0; contador< azonacen.length;contador++){
            totalcen += azonacen[contador];
        }
        System.out.println("Ocupadas Centro: " + totalcen );

    int[] totalzonas = new int[5];
    int mayor = 0;
    String Zona = "a";
        totalzonas[0] = totalocc;
        totalzonas[1] = totalori;
        totalzonas[2] = totalnor;
        totalzonas[3] = totalsur;
        totalzonas[4] = totalcen;
        System.out.println(Arrays.toString(totalzonas));
        for (int x = 0; x<totalzonas.length; x++) {
            if (totalzonas[x] > mayor) {
                mayor = totalzonas [x];

                if (mayor == totalocc){
                    Zona=Zona.replaceAll("a","Occidental");
                }
                if (mayor == totalori){
                    Zona=Zona.replaceAll("a", "Oriental");
                }
                if (mayor == totalnor){
                    Zona=Zona.replaceAll("a", "Norte");
                }
                if (mayor == totalsur){
                    Zona=Zona.replaceAll("a", "Sur");
                }
                if (mayor == totalcen){
                    Zona=Zona.replaceAll("a", "Cental");
                }
            }

        }
        System.out.println("La Zona con mas expositores es: " + Zona + " con " + mayor + " expositores.");
        return ("La Zona con mas expositores es: " + Zona + " con " + mayor + " expositores.");
    }
    /**
    ...
    public String metodo2() {
        ...
        String puntoequilibrio = "a";
            if (porcentajeOcupacion() >= 60) {
                puntoequilibrio=puntoequilibrio.replace("a", "El punto de equilibrio se ha cumplido");
            }
            else {
            puntoequilibrio=puntoequilibrio.replace("a", "El punto de equilibrio no se ha cumplido");
        }
        System.out.println(porcentajeOcupacion());
